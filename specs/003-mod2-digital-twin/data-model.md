...
- **module_number** (string): Module identifier (II for Module 2)
...
- **part_id** (string): Reference to Part III (Digital Twin)
...
### Relationships
- Belongs to one Module (Module 2)
...
- module_number must be "II" for Module 2
- chapter_number must be unique within the module
- topics array must not be empty
- all required sections must be present
- part_id must reference an existing Part III

## Entity: SimulationToolRequirement
...
### Relationships
- Associated with one Module (Module 2)
- Referenced by multiple ModuleChapters that use the tool
- May have dependencies on other SimulationToolRequirements

### Validation Rules
- name must be unique within the module
- version must follow semantic versioning
- module_id must reference Module 2
...
## Entity: SimulationExample

### Attributes
- **id** (string): Unique identifier for the simulation example
...
- **simulation_tool** (string): Primary tool used (Gazebo, Unity, or both)
...
### Relationships
- Belongs to one ModuleChapter
- Multiple SimulationExamples may exist within one chapter
- May reference external resources or dependencies

### Validation Rules
- simulation_tool must be Gazebo, Unity, or both
